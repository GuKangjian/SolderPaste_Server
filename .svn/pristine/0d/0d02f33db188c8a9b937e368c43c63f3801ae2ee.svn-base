...
namespace _Common
{
    ...
    public class JsonHelper
    {
        ...
        public static string JsonSerializerBySingleData<T>(T t)
        {
            ...
            string p = @"\\/Date\((\d+)\+\d+\)\\/";
            //   string p = @"\d{4}-\d{2}-\d{2}\s\d{2}:\d{2}:\d{2}";
            MatchEvaluator matchEvaluator = new MatchEvaluator(ConvertJsonDateToDateString);//ConvertDateStringToJsonDate
            //   MatchEvaluator matchEvaluator = new MatchEvaluator(ConvertDateStringToJsonDate);//ConvertDateStringToJsonDate
            Regex reg = new Regex(p);
            jsonString = reg.Replace(jsonString, matchEvaluator);
            return jsonString;
        }
        ...
        /// 序列化单个对象
        /// </summary>
        public static string JsonSerializerBySingleData_1<T>(T t)
        {
            DataContractJsonSerializer ser = new DataContractJsonSerializer(typeof(T));
            MemoryStream ms = new MemoryStream();
            ser.WriteObject(ms, t);
        ...
        /// <param name="table">Datatable对象</param> 
        /// <returns>Json字符串</returns> 
        public static string ToJsonSingle(DataTable dt)
        {
            StringBuilder jsonString = new StringBuilder();
            jsonString.Append("{");
            DataRowCollection drc = dt.Rows;
